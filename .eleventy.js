...
module.exports = function (eleventyConfig) {
    ...
    eleventyConfig.addCollection('postsByCategory', (collection) => {
        ...
        const groups = posts.reduce(
            (acc, item) => ((acc[item.data.category] = [...(acc[item.data.category] || []), item]), acc),
            {}
        );
        return Object.keys(groups)
            .sort()
            .reduce((p, c) => ((p[c] = groups[c]), p), {});
    });
    ...
    eleventyConfig.addPairedShortcode(
        'callout',
        (content) => `<div class='post__callout'>${markdownLibrary.render(content.trim())}</div>`
    );
