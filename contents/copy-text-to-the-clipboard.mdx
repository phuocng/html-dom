...
In order to do that, we [create](https://phuoc.ng/collection/html-dom/create-an-element) a fake `textarea` element with value as `text`. Next, we [select](https://phuoc.ng/collection/html-dom/trigger-an-event) the content and execute the "Copy" command.
